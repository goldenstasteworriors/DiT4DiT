...
2. 双臂从 LowState 实测角出发，以同一条 minimum-jerk 曲线移动至初始化数据帧。
   ...
   `[29:35]` 为左手记录状态、`[35:41]` 为右手记录状态。为复现所选帧的物理手姿，双手
   初始化命令和 READY 目标默认都使用这12维 `observation.state`；这对
   `grab_red_bottle_test` 很重要，因为其拇指旋转状态为 `0`，而同帧 `action.wbc` 为 `1`，
   直接重放 action 会得到不同的拇指姿态。仍可用 `--initial-left-hand-state`、
   `--initial-right-hand-state` 和 `--initial-hand-command` 显式覆盖。启动日志会同时打印
   observation、action、READY 目标、完整文件、帧号、时间戳和解析出的初始化值。
...
右臂和右手播放推理轨迹。左臂保持修正后的初始姿态，左手保持对应 episode 的首帧命令。
脚本默认以10 Hz发布限速后的 `rt/inspire/cmd`，由单独的 DDS/Modbus bridge 执行：
...
所有关节误差需连续2秒保持在 `0.1 rad` 内才进入 `READY`。内环使用
