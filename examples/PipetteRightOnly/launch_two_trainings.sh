...
CUDA_VISIBLE_DEVICES=2,4 nohup "${CONDA}" run -n dit4dit accelerate launch \
...
CUDA_VISIBLE_DEVICES=5,6 nohup "${CONDA}" run -n dit4dit accelerate launch \
  --config_file "${ACCELERATE_CONFIG}" \
...
echo "joint training: GPUs 2,4"
echo "wrist-delta training: GPUs 5,6"
