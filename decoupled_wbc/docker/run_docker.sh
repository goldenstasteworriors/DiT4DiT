...
#   --clean           Clean containers for current user/config
#   --clean-all       Clean ALL gr00t_wbc containers (requires confirmation)
#   --deploy          Run in deploy mode
#   --exec <name/id>  Execute bash in specific container by name or ID
...
function clean_all_containers {
    echo ""
    echo "============================================================"
    echo "  CLEAN ALL GR00T_WBC CONTAINERS"
    echo "============================================================"
    echo ""

    # Find all gr00t_wbc containers
    ALL_CONTAINERS=$(sudo docker ps -a --format '{{.Names}}' | grep "^gr00t_wbc-" || true)

    if [ -z "$ALL_CONTAINERS" ]; then
        echo "No gr00t_wbc containers found."
        echo ""
        return
    fi

    echo "Found the following gr00t_wbc containers:"
    echo ""
    ...
    for container in $ALL_CONTAINERS; do
        # Get container status and image
    ...
    echo "============================================================"
    echo ""
    echo "WARNING: This will stop and remove ALL gr00t_wbc containers."
    echo ""
    ...
    echo ""
    echo "All gr00t_wbc containers cleaned!"
    echo ""
}
...
# Execute into specific container if requested
if [ -n "$EXEC_CONTAINER" ]; then
    echo "Executing bash in container: $EXEC_CONTAINER"

    # Check if container exists
    if ! sudo docker ps -a --format '{{.Names}} {{.ID}}' | grep -qE "^${EXEC_CONTAINER}|${EXEC_CONTAINER}$"; then
        echo "Error: Container '$EXEC_CONTAINER' not found"
        echo ""
        echo "Available gr00t_wbc containers:"
        sudo docker ps -a --format 'table {{.Names}}\t{{.ID}}\t{{.Status}}' | grep gr00t_wbc || echo "  No gr00t_wbc containers found"
        exit 1
    fi

    # Start container if it's stopped
    CONTAINER_STATUS=$(sudo docker inspect --format='{{.State.Status}}' "$EXEC_CONTAINER" 2>/dev/null)
    if [ "$CONTAINER_STATUS" = "exited" ] || [ "$CONTAINER_STATUS" = "created" ]; then
        echo "Container is stopped, starting it..."
        sudo docker start "$EXEC_CONTAINER" > /dev/null
    fi

    # Exec into container
    sudo docker exec -it "$EXEC_CONTAINER" /bin/bash
    exit 0
fi
