...
class Custom:
    ...
    def Init(self):
        ...
        print("[DEBUG] Checking motion mode...")
        status, result = self.msc.CheckMode()
        print(f"[DEBUG] CheckMode returned: status={status}, result={result}")

        if result is None:
            print(f"\n[ERROR] Motion switcher service call failed (status code: {status})")
            print("\nDiagnostics:")
            print("  1. Check if robot is powered on and connected to network")
            print("  2. Check if you can ping the robot: ping <robot_ip>")
            print("  3. Check DDS environment variables:")
            ...
            print("  4. Check if motion_switcher service is running on robot")
            print("\nTo fix:")
            print("  - Ensure robot is on same network")
            print("  - Check DDS configuration matches robot settings")
            print("  - Restart unitree_dds_bridge on robot if needed")
            raise RuntimeError(f"Cannot communicate with robot (status: {status})")

        print(f"[DEBUG] Current mode: {result.get('name', 'none')}")
        while result.get('name'):
            print(f"[INFO] Releasing current mode: {result['name']}")
            self.msc.ReleaseMode()
            status, result = self.msc.CheckMode()
            if result is None:
                raise RuntimeError(f"CheckMode failed during release (status: {status})")
            time.sleep(1)

        print("[DEBUG] Motion mode is now free")
