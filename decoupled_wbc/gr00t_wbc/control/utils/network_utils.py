...
def resolve_interface(interface: str) -> tuple[str, str]:
    ...
    if interface == "sim":
        # Use a multicast-capable interface for simulation instead of loopback
        # Try to find enp4s0 first (common Ethernet interface with multicast)
        interfaces = get_network_interfaces()
        
        # Prefer enp4s0 if available (common Ethernet with multicast)
        if "enp4s0" in interfaces:
            return "enp4s0", "sim"
        
        # Otherwise find any interface with multicast capability (not lo)
        for iface, ip_list in interfaces.items():
            if iface not in ["lo", "lo0"] and ip_list:
                return iface, "sim"
        
        # Fallback to loopback if nothing else available (will fail but at least tries)
