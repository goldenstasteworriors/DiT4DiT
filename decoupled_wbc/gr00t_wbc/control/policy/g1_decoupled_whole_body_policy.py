...
class G1DecoupledWholeBodyPolicy(Policy):
    ...
    def __init__(
        ...
        self.latest_left_fingers = None  # Store latest finger data from teleop
        self.latest_right_fingers = None
    ...
    def set_goal(self, goal):
        ...
        # Store finger data for ALOHA gripper control (not part of interpolation)
        if "left_fingers" in goal:
            self.latest_left_fingers = goal["left_fingers"]
        if "right_fingers" in goal:
            self.latest_right_fingers = goal["right_fingers"]
    ...
    def get_action(self, time: Optional[float] = None):
        ...
        # print(f"[DEBUG g1_decoupled_whole_body_policy] upper_body_action keys: {upper_body_action.keys()}")
        ...
        # Check if torso_orientation_rpy is provided from teleop (e.g., Pico controller)
        torso_orientation_rpy_from_teleop = upper_body_action.get("torso_orientation_rpy", None)

        # Compute torso orientation relative to waist, to pass to lower body policy
        # Use teleop value if available, otherwise compute from FK
        if torso_orientation_rpy_from_teleop is not None:
            torso_orientation_rpy = torso_orientation_rpy_from_teleop
        else:
        ...
        # Pass through finger data from teleop (stored in set_goal, needed for ALOHA trigger control)
        action = {"q": q}
        if self.latest_left_fingers is not None:
            action["left_fingers"] = self.latest_left_fingers
            # print(f"[DEBUG g1_decoupled_whole_body_policy] Passing left_fingers: {action['left_fingers']}")
        if self.latest_right_fingers is not None:
            action["right_fingers"] = self.latest_right_fingers
            # print(f"[DEBUG g1_decoupled_whole_body_policy] Passing right_fingers: {action['right_fingers']}")
