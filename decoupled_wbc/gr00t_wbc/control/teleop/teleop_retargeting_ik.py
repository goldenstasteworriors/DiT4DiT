...
class TeleopRetargetingIK(Policy):
    ...
    def _inverse_kinematics(
        ...
        if left_hand_target_pose is not None and self.left_hand_ik_solver is not None:
        ...
        if right_hand_target_pose is not None and self.right_hand_ik_solver is not None:
    ...
    def set_current_pose(self, q: np.ndarray):
        """Set the IK solver's current pose to match actual robot state.

        Call this after reset to sync IK with the actual robot pose.
        This also syncs the body IK solver's internal configuration.
        """
        self._most_recent_q = q.copy()
        # Sync the body IK solver's internal configuration
        if self.using_reduced_robot_model:
            # Extract the reduced (active) joints from the full configuration
            reduced_q = self.body.full_to_reduced_configuration(q)
            # Clip to IK solver's joint limits to avoid Pink's limit check error
            reduced_q = self.body.clip_configuration(reduced_q)
            self.body_ik_solver.configuration.q = reduced_q
        else:
            clipped_q = self.body.clip_configuration(q)
            self.body_ik_solver.configuration.q = clipped_q
        self.body_ik_solver.configuration.update()
