    ...
    def __init__(
        ...
        if self.enable_real_device:
            if body_control_device == "pico":
                ...
                self.body_streamer.start_streaming()
            elif body_control_device == "keyboard":
                from gr00t_wbc.control.teleop.streamers.keyboard_streamer import (
                    KeyboardArmStreamer,
                )

                self.body_streamer = KeyboardArmStreamer()
                self.body_streamer.start_streaming()
            else:
                self.body_streamer = None

            if hand_control_device and hand_control_device != body_control_device:
                if hand_control_device == "pico":
                    from gr00t_wbc.control.teleop.streamers.pico_streamer import PicoStreamer
