...
class BaseConfig(ArgsConfigTemplate):
    ...
    hand_type: Literal["three_finger", "aloha"] = "aloha"
    """Type of hand hardware to use. Options: three_finger (7-DOF dexterous), aloha (1-DOF gripper, default)."""
    ...
    If None, defaults to upper body joints when enabled."""
    ...
    """Enable data collection (default: True, use --no-data_collection to disable)"""

    data_collection_frequency: int = 30
    ...
    """Camera to use for rendering. Options: frontview, agentview, robot0_oak_egoview, etc."""
    ...
    # Inference settings (used when upper_body_operation_mode="inference")
    inference_host: str = "localhost"
    """Host address of the GR00T inference server."""

    inference_port: int = 5555
    """Port of the GR00T inference server."""

    inference_use_http: bool = False
    """Use HTTP instead of ZMQ for inference server communication."""
    ...
    """Number of predicted actions to execute before re-querying the model."""

    inference_frequency: float = 10.0
    """Frequency of inference queries (Hz)."""

    inference_camera_key: str = "ego_view"
...
class TeleopConfig(BaseConfig):
    ...
    """Path to compact format replay data (new format with arm+gripper+rpy+height)."""
    ...
    teleop_frequency: float = 30  # Override to be float instead of int
...
class DataExporterConfig(BaseConfig, ComposedCameraClientConfig):
    ...
    """Size of the state. Auto-computed based on hand_type if not specified.
    - three_finger: 43 DOF (29 body + 14 hand)
    - aloha: 31 DOF (29 body + 2 gripper)
    """

    action_dim: Optional[int] = None
    """Size of the action. Auto-computed based on hand_type if not specified.
    - three_finger: 43 DOF (29 body + 14 hand)
    - aloha: 31 DOF (29 body + 2 gripper)
    """

    def __post_init__(self):
        """Initialize dimensions based on hand_type if not explicitly set."""
        super().__post_init__()
        # Compute DOF based on hand_type
        # three_finger: 29 body + 14 hand (7 per hand) = 43 DOF
        # aloha: 29 body + 2 gripper (1 per gripper) = 31 DOF
        dof = 43 if self.hand_type == "three_finger" else 31
...
class SyncSimDataCollectionConfig(ControlLoopConfig, TeleopConfig):
    ...
    body_control_device: str = "dummy"
    """Device to use for body control. Options: keyboard, dummy, vive, iphone, leapmotion, joycon, pico."""

    hand_control_device: Optional[str] = "dummy"
    """Device to use for hand control. Options: dummy, manus, joycon, iphone, pico."""
...
class DeploymentConfig(BaseConfig, ComposedCameraClientConfig):
    ...
    ego_camera: str = "realsense"
    """Camera type for ego view: 'realsense' (on Jetson) or 'odin' (on local host)"""
