...
class StateActionPadTransform(ModalityTransform):
    """
    Class for state or action padding transform.

    Args:
        apply_to (list[str]): The keys in the modality to load and transform.
        max_lengths (dict[str, int]): The maximum length for each key. If a key is not in the dict,
    ...
    def apply(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        actions = np.pad(actions, ((0, 0), (0, max_action_dim - n_action_dims)), "constant")

        # Create mask: [T, max_action_dim]
        actions_mask = np.zeros((n_action_tokens, max_action_dim), dtype=bool)
        actions_mask[:, :n_action_dims] = True

        data["state"] = state
        data["state_mask"] = state_mask
        data["actions"] = actions
        data["actions_mask"] = actions_mask
        return data
